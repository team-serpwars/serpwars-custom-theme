...
class Serpwars_Welcome_Base {
    ...
    function __construct(){
        $theme              = wp_get_theme();
        $this->theme_id  = $theme->get('Name');
        $this->page_slug = 'serpwars';
    ...
    protected function init_actions(){
        ...
        add_action( 'serpwars_admin_welcome_menu_args', array( $this, 'menu_badge_count'     ) );
    ...
    public function get_admin_menu_args(){
        $menu_name =  __( 'Theme Panel', 'serpwars');

        return apply_filters( 'serpwars_admin_welcome_menu_args', array(
            'name'          => $menu_name,
            'title'         => __('Welcome', 'serpwars'),
    ...
    public function get_sections(){
        if( empty( $this->sections ) ){
            $this->sections = apply_filters( 'serpwars_admin_welcome_sections', array() );
        }
    ...
    protected function the_header( $type ){
        ...
        $welcome_page_title  = AUXIN_NO_BRAND ? __('Welcome','serpwars') : sprintf( __( 'Welcome to %s Theme', 'serpwars' ) . '</strong>',  '<strong>'. THEME_NAME_I18N );
        /**
        ...
        */
        $welcome_page_title = apply_filters( 'serpwars_welcome_page_title', $welcome_page_title, $type );
        ...
        <div class="aux-welcome-header">
            ...
            <div class="aux-welcome-subtitle">
                <?php
                    echo '<span class="aux-welcome-desc-meta">' . sprintf( esc_html__('Version %s', 'serpwars'), THEME_VERSION ) . '</span>';
                    if( function_exists('serpwars_is_activated') &&  serpwars_is_activated() ){
                        printf( '<span class="aux-is-activated aux-welcome-desc-meta"> %s </span>', esc_html__('Activated', 'serpwars') );
                        printf( '<a class="aux-welcome-desc-meta-link aux-welcome-desc-meta" href="https://docs.serpwars.pro/article/174-transfer-license" target="_blank">%s</a>', esc_html__('How to deactivate?', 'serpwars') );
                    }
            ...
            <?php
                ...
                echo apply_filters( 'serpwars_display_theme_badge', $theme_badge );
            ?>
        </div>
        <?php
    }
    ...
    public function render(){
        ...
        <div class="wrap aux-welcome aux-welcome-page-<?php echo esc_attr( $tab ); ?>">
            <div class="aux-welcome-inner">
                <?php do_action( 'serpwars_admin_welcome_before_header' ); ?>
                <?php $this->the_header( $tab ); ?>
                <?php do_action( 'serpwars_admin_welcome_after_header' ); ?>
    ...
    public function menu_badge_count( $theme_menu_args ){
        if( $update_count = apply_filters( 'serpwars_theme_menu_update_count', 0 ) ){
    ...
    protected function the_content( $type ){
        ...
        do_action( 'serpwars_admin_before_welcome_section_content', $type, $sections );
        ...
        do_action( 'serpwars_admin_after_welcome_section_content', $type, $sections );
    }
